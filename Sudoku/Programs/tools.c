...
/* Applique toutes les contraintes associees aux cases de la pile */

int contrainte_unicite(GRILLE g, PILE_CASE *pile)
{
	CASE *c;
	int verif;
	...
	{
		c = *(pile->cases+pile->nb_cases--);
		verif = contrainte_unicite_case(g, pile, c);
		if(!verif)
			return 0;
	}
	return 1;
